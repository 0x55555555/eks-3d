...
void main(void)
  ...
  vec4 diffuse = texture(rsc0, texOut);
  vec3 normal = normalMat * (normalize(texture(rsc1, texOut).xyz) - 0.5) * 2.0;
  vec4 specular = vec4(1, 1, 1, 0);
  vec4 colour = vec4(0.05, 0.05, 0.05, 0);

  float dist = length(mvPos - lightPosition);
  ...
  if (NdotL > 0.0)
    ...
    vec3 eye = normalize(-mvPos);
    vec3 toLight = lightPosition - mvPos;
